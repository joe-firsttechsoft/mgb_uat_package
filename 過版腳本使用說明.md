...
**環境變數**：`MGBFEP_PROJECT_DIR` — 兆豐 FEP 專案（`mgbfep`）的 worktree 路徑，預設 `$HOME/Repo/idea_clone/mgbfep-UAT`。所有 commit、tag 都是對這個 repo 操作，不是對 `MGB_UAT` 這個腳本所在的 repo。

**分支檢查**：腳本最前面定義 `EXPECTED_BRANCH_PATTERN='^FEP_[0-9][0-9.-]*(_UAT|_SIT)?$'`，執行時會檢查 `MGBFEP_PROJECT_DIR` 專案目前所在分支是否符合 `FEP_<版本>_UAT` / `FEP_<版本>_SIT` / `FEP_<版本>`（PROD，無結尾字樣）這個格式。若不符合（例如切到 `develop`、`main` 之類的分支），會印出警告並詢問是否仍要繼續（`y` 才會繼續，其餘輸入一律取消，不會動到 `outputs/` 或建立 tag）。版本升級時（如 1-3-2 → 1-3-3）不需要再改腳本。

**分支與 tag 前綴對應**：依目前分支名稱判斷（`case "$CURRENT_BRANCH" in *UAT) ... ;; *SIT) ... ;; FEP_[0-9]*) ... ;; esac`）：

| 分支 | tag 前綴 | 找前次 tag / 建立新 tag |
|---|---|---|
| `*_UAT` 結尾 | `uat/release` | 例如 `uat/release/260703` |
| `*_SIT` 結尾 | `sit/release` | 例如 `sit/release/260703` |
| `FEP_<版本>`（無結尾字樣，PROD 正式分支，由 UAT 分支合併而來） | `prod/release` | 例如 `prod/release/260703` |
...
5. 依目前分支決定的 tag 前綴，在 `MGBFEP_PROJECT_DIR` 的 `HEAD` 上打 tag `<前綴>/<今天 yyMMdd>`（若 tag 已存在則跳過，不會報錯；分支非 UAT/SIT/PROD 格式則不建立），做為**下一次**執行 `release.sh` 時自動偵測的前次過版點

> 因為第 5 步會自動打 tag，正常情況下往後執行 `release.sh` **不需要再帶參數**，腳本會自己接續上一次的 tag（只要分支符合 UAT、SIT 或 PROD 格式）。
...
- 輸出檔名：`outputs/<今天 yyyyMMdd>-P1-3 兆豐UAT異動項目.xlsx`（日期格式比照 `outputs.0702/20260623-P1-2 兆豐UAT異動項目.xlsx` 這類既有範例檔，檔名中的版本代號已由 P1-2 改為 P1-3）
...
3. **其他模組**：先判斷來源檔案——若是 `PACKAGE_DEFS` 定義的封裝 tar.gz（見下表），更新說明固定「全模組更新」；若不是 tar.gz（一般原始碼模組，來源檔案維持 `source/fep/<module>`），更新說明**留空白**，不再列檔案數差異。
  ...
  兆豐 UAT 專案實際部署的封裝 tar.gz（`PACKAGE_DEFS`，已用 `MGBFEP_PROJECT_DIR` 專案的 pom.xml / assembly 設定核對過，2026-08-05 補上先前漏列的 eatm/cbs/camp/log/ibsmq/sendfisc2160）：

  | Server | 封裝檔（來源檔案） | 對應 Maven 模組 |
  |---|---|---|
  | FEP-SERVER-ATM | `fep-server-bin-atm.tar.gz` | `fep-server`（8 種變種全列：atm/fisc/mb/hce/lateresponse/eatm/cbs/camp） |
  ...
  | FEP-SERVER-EATM | `fep-server-bin-eatm.tar.gz` | `fep-server` |
  | FEP-SERVER-CBS | `fep-server-bin-cbs.tar.gz` | `fep-server` |
  | FEP-SERVER-CAMP | `fep-server-bin-camp.tar.gz` | `fep-server` |
  | FEP-SERVICE-APPMON | `fep-service-bin-appmon.tar.gz` | `fep-service`（6 種變種全列：ems/log/appmon/ibsmq/cbstimeoutrerun/sendfisc2160） |
  ...
  | FEP-SERVICE-LOG | `fep-service-bin-log.tar.gz` | `fep-service` |
  | FEP-SERVICE-IBSMQ | `fep-service-bin-ibsmq.tar.gz` | `fep-service` |
  | FEP-SERVICE-SENDFISC2160 | `fep-service-bin-sendfisc2160.tar.gz` | `fep-service` |
