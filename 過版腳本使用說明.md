...
| Sheet | 內容 | 篩選規則 |
|---|---|---|
| **異動清單** | 逐檔列出 Index／修改檔案／位置／異動類型／修改內容 | 排除「刪除」；副檔名限 `.java .xml .html .js .txt .jar .sql .css`（**不含 `.properties`**，properties 一律只出現在「設定檔」頁）；排除 `source/SIT套config`、`source/開發套config` 路徑；排除路徑中含有 `fep-release-note` 字串（不限開頭）；排除路徑包含 `src/test/` 的測試程式碼（檔名含 `BatchTaskUtil`、`AssemblyPropFileGenerator`、`ReleaseNoteGenerator`、`RemoveVersion` 關鍵字者例外保留，比照 `../mgb-vul/RemoveTest.ps1`） |
